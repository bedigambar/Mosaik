...
function findNearestColor(r: number, g: number, b: number, paletteId: string) {
  const p = PALETTES.find(x => x.id === paletteId) ?? PALETTES[0];
  let minD = Infinity, best = p.colors[0];
  for (const c of p.colors) {
    const dr = r - c[0], dg = g - c[1], db = b - c[2];
    const d = dr * dr + dg * dg + db * db;
    if (d < minD) { minD = d; best = c; }
  }
  return { cr: best[0], cg: best[1], cb: best[2] };
}
...
export function ditherImage(
  ...
  const sampleColor = (ox: number, oy: number) => {
    if (params.usePalette) {
      if (params.useSourceColor) {
        const sr = src[(clamp(Math.floor(oy * sy), 0, ih - 1) * iw + clamp(Math.floor(ox * sx), 0, iw - 1)) * 4];
        const sg = src[(clamp(Math.floor(oy * sy), 0, ih - 1) * iw + clamp(Math.floor(ox * sx), 0, iw - 1)) * 4 + 1];
        const sb = src[(clamp(Math.floor(oy * sy), 0, ih - 1) * iw + clamp(Math.floor(ox * sx), 0, iw - 1)) * 4 + 2];
        return findNearestColor(sr, sg, sb, params.paletteId);
      } else {
        // Map 1-bit to palette extremes or specific palette colors
        const p = PALETTES.find(x => x.id === params.paletteId) ?? PALETTES[0];
        const c = p.colors[p.colors.length - 1]; // Assume last is lightest for dots
        return { cr: c[0], cg: c[1], cb: c[2] };
      }
    }
    return params.useSourceColor ? {
  ...
  const getGridCoords = (ox: number, oy: number, gy: number) => {
    let x = ox, y = oy;
    if (params.gridType === "hex") {
      if (gy % 2 === 1) x += step / 2;
      y = gy * (step * 0.866); // True hex row spacing
    } else if (params.gridType === "offset") {
      if (gy % 2 === 1) x += step / 2;
    }
    return { x, y };
  };
  ...
  if (params.algorithm === "ordered") {
    for (let gy = 0; gy < outputHeight / step; gy++) {
      for (let gx = 0; gx < outputWidth / step; gx++) {
        const { x, y } = getGridCoords(gx * step, gy * step, gy);
        const v = gray[clamp(Math.floor(y * sy), 0, ih - 1) * iw + clamp(Math.floor(x * sx), 0, iw - 1)];
        if (v < (BAYER_8x8[Math.floor(y / step) % 8][Math.floor(x / step) % 8] / 64) * 255) {
          const i = 1 - v / 255, rad = params.dotMinRadius + i * (params.dotMaxRadius - params.dotMinRadius);
          dots.push({ x, y, r: rad, ...sampleColor(x, y) });
        }
      }
    }
    ...
    for (let gy = 0; gy < gh; gy++) for (let gx = 0; gx < gw; gx++) {
      const { x, y } = getGridCoords(gx * step, gy * step, gy);
      const px = clamp(Math.floor(x * sx), 0, iw - 1), py = clamp(Math.floor(y * sy), 0, ih - 1);
...
export function drawDots(
  ...
  // Dots
  for (const d of dots) {
    if (params.useSourceColor || params.usePalette) {
      offCtx.fillStyle = d.cr !== undefined ? `rgb(${d.cr},${d.cg},${d.cb})` : params.dotColor;
    } else {
      offCtx.fillStyle = params.dotColor;
    }

    if (glyphImage && params.customGlyph) {
      const size = d.r * 2;
      offCtx.drawImage(glyphImage, d.x - d.r, d.y - d.r, size, size);
    } else {
  ...
  if (params.crtEffect) {
    ...
    // Subtle vignette
    const grad = offCtx.createRadialGradient(w / 2, h / 2, Math.max(w, h) * 0.4, w / 2, h / 2, Math.max(w, h) * 0.7);
...
export function generateInteractionCode(dots: DotCoord[], repelRadius: number, repelStrength: number): string {
  return `// Mosaik — Generated Interaction Code
...
export function generateReactCode(p: DitherParams, ap: Record<string, unknown>, mode: string, videoRender: string): string {
  ...
  return `import { Mosaik } from 'mosaik-react';
// npm install mosaik-react
