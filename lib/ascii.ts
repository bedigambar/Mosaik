...
export interface AsciiParams {
    charset: "detailed" | "blocks" | "pixel" | "minimal" | "braille" | "custom";
...
export const ASCII_CHARSETS: Record<string, string> = {
    ...
    minimal: "@:. ",
    braille: "Braille", // special handling in imageDataToAscii
};
...
export function imageDataToAscii(
    ...
    const isBraille = params.charset === "braille";

    // Braille characters are 2x4 dots. 
    // We treat each "cell" as one Braille character.
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            ...
            if (isBraille) {
                ...
                let rSum = 0, gSum = 0, bSum = 0, aSum = 0, totalCount = 0;

                for (let dotIdx = 0; dotIdx < 8; dotIdx++) {
                    const [dx, dy, bit] = dotPattern[dotIdx];
                    // Sample sub-area for this dot
                    const subX0 = Math.floor(x0 + (dx / 2) * (x1 - x0));
                    const subY0 = Math.floor(y0 + (dy / 4) * (y1 - y0));
                    const subX1 = Math.floor(x0 + ((dx + 1) / 2) * (x1 - x0));
                    const subY1 = Math.floor(y0 + ((dy + 1) / 4) * (y1 - y0));

                    let dotLumSum = 0, dotCount = 0;
                    for (let py = subY0; py <= subY1; py++) {
                        for (let px = subX0; px <= subX1; px++) {
                            const i = (clamp(py, 0, ih - 1) * iw + clamp(px, 0, iw - 1)) * 4;
                            const r = src[i], g = src[i + 1], b = src[i + 2], a = src[i + 3] / 255;
                            let l = (0.299 * r + 0.587 * g + 0.114 * b) * a + 255 * (1 - a);
                            l = clamp(l + params.brightness);
                            l = applyContrast(l, params.contrast);
                            l = applyGamma(l, params.gamma);
                            if (params.invertBrightness) l = 255 - l;
                    ...
                    const avgDotLum = dotLumSum / (dotCount || 1);
                    if (avgDotLum > 127) brailleCode |= bit;
                }
...
export function generateAsciiVideoCode(
    ...
    return `// Mosaik — ASCII Video Player
